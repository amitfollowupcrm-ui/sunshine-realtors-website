// Dummy Property Data Seeder
// Seeds properties for Mohali, Zirakpur, Kharar, Kasauli, and Panchkula

// Try to load dotenv if available
try {
  require('dotenv').config({ path: '.env.local' });
} catch (e) {
  // dotenv not installed, environment variables should be set directly
}
...
// High-quality compressed images from Unsplash (optimized for web)
...
function generateProperty(city, ownerId) {
  ...
  let basePrice = 3000000; // 30 lakh base
  ...
  if (category === PropertyCategory.RENT) basePrice = basePrice * 0.005; // Monthly rent
...
async function seedProperties() {
  ...
  try {
    ...
    for (const city of cities) {
      const count = randomInt(15, 25); // 15-25 properties per city
